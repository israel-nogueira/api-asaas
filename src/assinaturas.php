...
trait assinatura{
	...
	*/
		static public function novaAssinatura($_PARAM=null) {try{
			...
			//ˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑˑ
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, getEnv('ASAAS_AMBIENTE')."/api/v3/subscriptions");
	...
	*/
		static public function getAssinatura($_ID=null) {try{
			...
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, getEnv('ASAAS_AMBIENTE')."/api/v3/subscriptions/$_ID");
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_HEADER, false);
			curl_setopt($ch,CURLOPT_HTTPHEADER, array( "Content-Type: application/json","access_token:".getEnv('APIKEY')));
			$response = curl_exec($ch);
	...
	*/
		static public function listaAssinaturas($_PARAM=null) {try{
			...
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, getEnv('ASAAS_AMBIENTE')."/api/v3/subscriptions".$_QUERY);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
			curl_setopt($ch, CURLOPT_HEADER, FALSE);
	...
	*/
		static public function listaCobrancaAssinatura($_ID=null) {try{
			...
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL,getEnv('ASAAS_AMBIENTE')."/api/v3/subscriptions/$_ID/payments");
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_HEADER, false);
			curl_setopt($ch,CURLOPT_HTTPHEADER, array( "Content-Type: application/json","access_token:".getEnv('APIKEY')));
			$response = curl_exec($ch);
	...
	*/
		static public function getCarneAssinatura($_ID=null,$_PARAM=null) {try{
			...
			$_QUERY = (!is_null($_PARAM)) ? "?".http_build_query($_PARAM):"";
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, getEnv('ASAAS_AMBIENTE')."/api/v3/subscriptions/".$_ID."/paymentBook".$_QUERY);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_HEADER, false);
			curl_setopt($ch,CURLOPT_HTTPHEADER, array("access_token:".getEnv('APIKEY')));
			$response = curl_exec($ch);
